...
def get_multilevel_single_stock_level(
    ...
    prob = cp.Problem(obj, constraints)
    # prob.solve(solver=cp.GLPK_MI, verbose=False)
    # GLPK_MI can only solve LP and may get stuck in face of some SKU 
    ...
    while not is_done:
        if current_step % update_freq == 0:
            if isinstance(static_stock_levels, np.ndarray):
                #TODO: 需要check一下这里的逻辑
                if static_stock_levels.ndim == 2:
                    stock_levels = static_stock_levels
                else:
                    stock_levels = static_stock_levels[0]
                
                # in case for dynamic situation, where static_stock_levels is two dimension
                # elif static_stock_levels.ndim == 2:
                #     stock_levels = static_stock_levels[:, current_step // update_freq]
            else:
                # 上面一个给各层独立用，下面一个给各层联合用
...
# for multi level base stock test
def base_stock_test(env: gym.Wrapper, update_freq =7, static_stock_levels = None):
    env.reset()
    current_step = 0
    is_done = False
    ...
    while not is_done:
        if current_step % update_freq == 0:
            if isinstance(static_stock_levels, np.ndarray):
                ...
                stock_levels = get_multilevel_stock_level(env)
        for i in range(len(env.get_warehouse_list())):
            #TODO: just for the test of multilevel base stock
...
if __name__ == "__main__":
    ...
    for env_name in env_names:
        ...
        # balance, dynamic_stock_levels_list= base_stock(env)
        # env.render()
        # print(env_name, exp_name, balance)

        # dynamic_stock_levels_list = np.array(dynamic_stock_levels_list)
        # profit_list = []
        # lim = 20
        # for i in range(-lim,lim):
        #     print(i)
        #     temp_level = (dynamic_stock_levels_list*(1+0.01*i)).astype(int)
        #     env.reset()
        #     balance, _ = base_stock_test(env, static_stock_levels = temp_level)
        ...
        # plt.xlabel('adjust(%)')
        # plt.ylabel('profit')
        # savepath = os.path.join(env_name, exp_name, "output_multilevel", 'base stock profit.jpg')
        # plt.savefig(savepath)
        # ---------------------------------test for multilevel static-----------------------------

        # exp_name = "static_base_stock_warmup_horizon_all_training_set"
        # # exp_name = "static_base_stock_no_warmup_all_training_data"
        # vis_path = os.path.join("output_multilevel", env_name, exp_name)
        # env_train = make_env(env_name, wrapper_names=["OracleWrapperNoWarmup"], mode="train", vis_path=vis_path)
        # env_train.reset()
        # # static_stock_levels = get_stock_level(env_train, env_mode = "train")
        # # because we use NoWarmup, so env_mode could change to 'test'
        # static_stock_levels = get_multilevel_stock_level(env_train, env_mode = "test").reshape((-1, len(env_train.get_sku_list())))
        # print(static_stock_levels)
        # env_test = make_env(env_name, wrapper_names=["OracleWrapperNoWarmup"], mode="test", vis_path=vis_path)
        # env_test.reset()
        # balance, _ = base_stock(env_test, static_stock_levels = static_stock_levels)
        # # static_stock_levels = static_stock_levels.reshape((-1, len(env_test.get_sku_list())))
        # env_test.render()
        # print(env_name, exp_name, balance)
        ...
        # env_test.render()
        # print(env_name, exp_name, balance)
        # -----------------------------------------------get mean stock level in oracle wrapper setting----------------------------------
        ...
        # --------------------plot stock levels along time for single store static(OracleWrapper), dynamic(history) and dynamic21---------------------------------------
        # exp_name = "static(level independent)"
        # vis_path = os.path.join("output_multilevel", env_name, exp_name)
        # env_train = make_env(env_name, wrapper_names=["OracleWrapper"], mode="train", vis_path=vis_path)
        # env_train.reset()
        # static_stock_levels = get_stock_level(env_train, env_mode='train')
        # env_test = make_env(env_name, wrapper_names=["OracleWrapper"], mode="test", vis_path=vis_path)
        # balance, static_ind_stock_levels_list = base_stock(env_test, static_stock_levels = static_stock_levels)
        # env_test.render()
        # print(env_name, exp_name, balance)

        # exp_name = "static(level correlated)"
        # vis_path = os.path.join("output_multilevel", env_name, exp_name)
        # env_train = make_env(env_name, wrapper_names=["OracleWrapper"], mode="train", vis_path=vis_path)
        # env_train.reset()
        # static_stock_levels = get_multilevel_stock_level(env_train, env_mode='train')
        # env_test = make_env(env_name, wrapper_names=["OracleWrapper"], mode="test", vis_path=vis_path)
        # balance, static_cor_stock_levels_list = base_stock(env_test, static_stock_levels = static_stock_levels)
        # env_test.render()
        # print(env_name, exp_name, balance)
        
        # exp_name = "static(level correlated w/o warmup)"
        # vis_path = os.path.join("output_multilevel", env_name, exp_name)
        # env_train = make_env(env_name, wrapper_names=["OracleWrapperNoWarmup"], mode="train", vis_path=vis_path)
        # env_train.reset()
        # # no warmup so the env_mode here is test
        # static_stock_levels = get_multilevel_stock_level(env_train, env_mode = 'test')
        # env_test = make_env(env_name, wrapper_names=["OracleWrapperNoWarmup"], mode="test", vis_path=vis_path)
        # balance, static_cor_no_warmup_stock_levels_list = base_stock(env_test, static_stock_levels = static_stock_levels)
        # env_test.render()
        # print(env_name, exp_name, balance)
        ...
        # for i in range(len(env.get_warehouse_list())):
        #     ax[i].plot(static_ind_stock_levels_list[i])
        #     ax[i].plot(static_cor_stock_levels_list[i])
        #     ax[i].plot(static_cor_no_warmup_stock_levels_list[i])
        #     #ax[i].plot(static_cor_stock_levels_list[i])
        #     ax[i].plot(history_stock_levels_list[i])
        #     ax[i].plot(lookback21_stock_levels_list[i])
        #     ax[i].legend(['static(layer independent)', 'static(layer correlated)', 'static(layer correlated w/o warmup)', 'history', 'lookback21'])
        #     ax[i].set_title('warehouse {}'.format(i+1))
        # savepath = os.path.join("output_multilevel", env_name,'stock level.jpg')
        # fig.savefig(savepath)
        # --------------------stock levels along time for multi store static(independent),static(correlated),static(OracleWrapperNoWarmup), dynamic(history) and dynamic21---------------------------------------
        # exp_name = "static(level independent)"
        # vis_path = os.path.join("output_multilevel", env_name, exp_name)
        # env_train = make_env(env_name, wrapper_names=["OracleWrapper"], mode="train", vis_path=vis_path)
        # env_train.reset()
        ...
        # static_ind_stock_levels_list = np.array(static_ind_stock_levels_list)
        # static_cor_stock_levels_list = np.array(static_cor_stock_levels_list)
        # static_cor_no_warmup_stock_levels_list = np.array(static_cor_no_warmup_stock_levels_list)
        ...
        # fig,ax = plt.subplots(3,1)
        # for i in range(len(env.get_warehouse_list())):
        #     ax[i].plot(static_ind_stock_levels_list[i].sum(axis = -1))
        #     ax[i].plot(static_cor_stock_levels_list[i].sum(axis = -1))
        #     ax[i].plot(static_cor_no_warmup_stock_levels_list[i].sum(axis = -1))
        #     #ax[i].plot(static_cor_stock_levels_list[i])
        ...
        exp_name = "static(oracle)"
        vis_path = os.path.join("output_multilevel", env_name, exp_name)
        env_train = make_env(env_name, wrapper_names=["OracleWrapper"], mode="test", vis_path=vis_path)
        env_train.reset()
        static_stock_levels = get_stock_level(env_train, env_mode='test')
        env_test = make_env(env_name, wrapper_names=["OracleWrapper"], mode="test", vis_path=vis_path)
        balance, static_ind_stock_levels_list, GMV, reward, reward_info = base_stock_test(env_test, static_stock_levels = static_stock_levels)
        ...
        # exp_name = "static(oracle no warmup)"
        # vis_path = os.path.join("output_multilevel", env_name, exp_name)
        # env_train = make_env(env_name, wrapper_names=["OracleWrapperNoWarmup"], mode="test", vis_path=vis_path)
        # env_train.reset()
        # static_stock_levels = get_stock_level(env_train, env_mode='test')
        # env_test = make_env(env_name, wrapper_names=["OracleWrapperNoWarmup"], mode="test", vis_path=vis_path)
        # balance, static_ind_stock_levels_list, GMV, reward, reward_info = base_stock_test(env_test, static_stock_levels = static_stock_levels)
        ...
        # static_stock_levels = get_multilevel_stock_level(env_train, env_mode='train')
        # env_test = make_env(env_name, wrapper_names=["OracleWrapper"], mode="test", vis_path=vis_path)
        # balance, static_cor_stock_levels_list, GMV, reward, reward_info  = base_stock_test(env_test, static_stock_levels = static_stock_levels)
        # os.makedirs(vis_path, exist_ok=True)
        # analyze(env_test, reward, GMV, reward_info, os.path.join(vis_path, 'analysis.csv'))
        # summary(os.path.join(vis_path, 'analysis.csv'), os.path.join(vis_path,'summary.csv'))
        # env_test.render()
        ...
        # static_stock_levels = get_multilevel_stock_level(env_train, env_mode = 'test')
        # env_test = make_env(env_name, wrapper_names=["OracleWrapperNoWarmup"], mode="test", vis_path=vis_path)
        # balance, static_cor_no_warmup_stock_levels_list, GMV, reward, reward_info  = base_stock_test(env_test, static_stock_levels = static_stock_levels)
        # os.makedirs(vis_path, exist_ok=True)
        # analyze(env_test, reward, GMV, reward_info, os.path.join(vis_path, 'analysis.csv'))
        # summary(os.path.join(vis_path, 'analysis.csv'), os.path.join(vis_path,'summary.csv'))
        # env_test.render()
        ...
        # vis_path = os.path.join("output_multilevel", env_name, exp_name)
        # env = make_env(env_name, wrapper_names=["StaticWrapper"], mode="test", vis_path=vis_path)
        # balance, history_stock_levels_list, GMV, reward, reward_info = base_stock_test(env)
        # os.makedirs(vis_path, exist_ok=True)
        # analyze(env_test, reward, GMV, reward_info, os.path.join(vis_path, 'analysis.csv'))
        # summary(os.path.join(vis_path, 'analysis.csv'), os.path.join(vis_path,'summary.csv'))
        # env.render()
        ...
        # vis_path = os.path.join("output_multilevel", env_name, exp_name)
        # env = make_env(env_name, wrapper_names=["DynamicWrapper"], mode="test", vis_path=vis_path)
        # balance, lookback21_stock_levels_list, GMV, reward, reward_info  = base_stock_test(env)
        # os.makedirs(vis_path, exist_ok=True)
        # analyze(env_test, reward, GMV, reward_info, os.path.join(vis_path, 'analysis.csv'))
        # summary(os.path.join(vis_path, 'analysis.csv'), os.path.join(vis_path,'summary.csv'))
        # env.render()
