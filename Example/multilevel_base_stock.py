...
def stock_level_plotting(stock_level_list, legend_list, env, env_name):
    fig,ax = plt.subplots(env.warehouse_count, 1)
    for i in range(env.warehouse_count):
        for stock_level in stock_level_list:
            ax[i].plot(stock_level[i].sum(axis = -1))
        ax[i].legend(legend_list)
...
if __name__ == "__main__":
    ...
    for env_name in env_names:
        ...
        balance, oracle_stock_levels_list = multilevel_base_stock(env_test, static_stock_levels = static_stock_levels)
        ...
        static_stock_levels = get_multilevel_stock_level(env_train, env_mode = 'train')
        env_test = make_env(env_name, wrapper_names=["OracleWrapper"], mode="test", vis_path=vis_path)
        balance, static_stock_levels_list = multilevel_base_stock(env_test, static_stock_levels = static_stock_levels)
        os.makedirs(vis_path, exist_ok=True)
        env_test.render()
        print(env_name, exp_name, balance)
        print(np.mean(static_stock_levels,axis=1))
        ...
        balance, history_stock_levels_list = multilevel_base_stock(env)
        os.makedirs(vis_path, exist_ok=True)
        env.render()
        print(env_name, exp_name, balance)
        ...
        balance, lookback21_stock_levels_list = multilevel_base_stock(env)
        os.makedirs(vis_path, exist_ok=True)
        env.render()
        print(env_name, exp_name, balance)
        ...
        stock_level_list = [oracle_stock_levels_list,static_stock_levels_list,history_stock_levels_list,lookback21_stock_levels_list]
        legend_list = ['oracle','static','history','lookback21']
        stock_level_plotting(stock_level_list,legend_list,env, env_name)