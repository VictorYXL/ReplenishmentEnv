...
class MAPPOMACWITHBASESTOCK:
    ...
    def select_actions(self, ep_batch, t_ep, t_env, bs=slice(None), test_mode=False):
        ...
        # TODO:更改avail_actions
        # avail_actions = avail_actions[:,:int(self.n_agents/3),:]
    ...
    def forward(self, ep_batch, t, t_env, test_mode=False):
        # TODO:forward中也更改了input
        agent_inputs = build_actor_inputs(self.input_seq_str, ep_batch, t)
        #agent_inputs = agent_inputs[:,:int(self.n_agents/3),:]
    ...
    def init_hidden(self, batch_size):
        ...
        if self.hidden_states is not None:
            self.hidden_states = self.hidden_states.unsqueeze(0).expand(
                #batch_size, int(self.n_agents/3), -1
    ...
    def _build_inputs(self, batch, t):
        ...
        if self.args.obs_agent_id:
            inputs.append(
                #torch.eye(int(self.n_agents/3), device=batch.device)
                torch.eye(self.n_agents, device=batch.device)
                .unsqueeze(0)
                .expand(bs, -1, -1)
            )

        #inputs = torch.cat([x.reshape(bs, int(self.n_agents/3), -1) for x in inputs], dim=-1)
    ...
    def _get_input_shape(self, scheme):
        ...
        if self.args.obs_agent_id:
            #input_shape += int(self.n_agents/3)
