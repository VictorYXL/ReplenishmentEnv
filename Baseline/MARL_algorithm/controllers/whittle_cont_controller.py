...
class WhittleContinuousMAC(object):
    ...
    def select_actions(self, ep_batch, t_ep, t_env, lbdas, bs=slice(None), test_mode=False):
        ...
        lbdas = lbdas.unsqueeze(1).unsqueeze(2).expand_as(agent_outputs)
        chosen_actions = (agent_outputs > lbdas).sum(dim=-1)
