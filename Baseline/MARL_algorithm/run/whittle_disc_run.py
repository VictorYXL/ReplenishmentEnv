...
def run_sequential(args, logger):
    ...
    if args.use_n_lambda:
        scheme = {
            ...
            "reward": {"vshape": (args.n_lambda,)},
            "individual_rewards": {"vshape": (args.n_lambda,), "group": "agents"},
    ...
    while runner.t_env <= args.t_max:
        ...
        if runner.t_env >= last_test_T + args.test_interval:
            ...
            tiny_lbda_list = [0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50]
            if runner.t_env % 100000 < args.test_interval:
                lbda_list = np.arange(51)
            else:
                lbda_list = tiny_lbda_list
            for lbda_index in lbda_list:
                for storage_capacity in [20000, 10000, 5000, 2500]:
                    stats, val_lambda_return, val_old_return = val_runner.run(
                        test_mode=True, lbda_index=lbda_index, storage_capacity=storage_capacity)
                    w_stats, w_val_lambda_return, w_val_old_return = w_val_runner.run(
                        test_mode=True, lbda_index=lbda_index, storage_capacity=storage_capacity)
                    log_dict.append({
                        'step': runner.t_env,
                        'lbda_index': lbda_index,
                        'lambda': lbda_index / 5,
            ...
            log_items = log_df.loc[(log_df['storage_capacity'] == 20000) \
                & log_df['lbda_index'].apply(lambda x: x in tiny_lbda_list)]
    ...
    for i_test in range(n_test_runs):
        ...
        for lbda_index in np.arange(51):
            for storage_capacity in [20000, 10000, 5000, 2500]:
                _, test_lambda_return, test_old_return = test_runner.run(
                    test_mode=True, lbda_index=lbda_index, storage_capacity=storage_capacity)
                log_record.update({
                    'val_return_lbda_l{}_c{:03d}k'.format(lbda_index, storage_capacity // 1000): test_lambda_return,
                    'val_return_old_l{}_c{:03d}k'.format(lbda_index, storage_capacity // 1000): test_old_return})
