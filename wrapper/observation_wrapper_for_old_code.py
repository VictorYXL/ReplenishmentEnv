...
class ObservationWrapper4OldCode(gym.Wrapper):
    def __init__(self, env: gym.Env) -> None:
        ...
        # TODO:在这里得到warehouse的数目！等于env.warehouse_count
        ...
        embedding = nn.Embedding(self.n_skus, self.embedding_dim, max_norm=self.embedding_dim)
        agent_id = torch.LongTensor(np.arange(self.n_skus))
        embedding_agent_id = embedding(agent_id).detach().cpu().numpy()
        self.embedding_agent_id = np.concatenate([embedding_agent_id for i in range(self.n_warehouses)]).reshape((self.n_warehouses, self.n_skus, -1))
        # TODO：先写一个固定长度的embedding
        self.state_dim = 0
        self.state_info = ["local_info", "global_info", "agent_specific_info"]# "rank_info", "mean_field"]
        ...
        if "rank_info" in self.state_info:
            self.state_dim += 7
        if "agent_specific_info" in self.state_info:
            self.state_dim += env.warehouse_count
            self.state_dim += self.embedding_dim
