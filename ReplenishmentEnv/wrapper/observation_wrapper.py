...
class ObservationWrapper(gym.Wrapper):
    ...
    def get_state_v1(self) -> np.array:
        ...
        state_list = []
        storage_capacity = np.array([self.supply_chain[warehouse, "capacity"] for warehouse in self.supply_chain.get_warehouse_list()])
        unit_storage_cost = np.array([self.supply_chain[warehouse, "unit_storage_cost"] for warehouse in self.supply_chain.get_warehouse_list()])
        if "local_info" in self.state_info:
            # is_out_of_stock
        ...
        if "global_info" in self.state_info:
            ...
            # in_stock_profit
            state_list.append((np.ones((self.n_warehouses, self.n_skus)) * (self.env.agent_states["all_warehouses", "in_stock"] * sku_profit).sum() / \
                        ((self.env.agent_states["all_warehouses", "in_stock"].sum() + 1) * price_normalize))[:, :, np.newaxis])

            # remain_capacity
            state_list.append((np.ones((self.n_warehouses, self.n_skus)) * (storage_capacity - self.env.agent_states["all_warehouses", "in_stock"].sum()) /
                                            storage_capacity)[:, :, np.newaxis])

            # intransit_sum
            state_list.append((np.ones((self.n_warehouses, self.n_skus)) * self.env.agent_states["all_warehouses", "in_transit"].sum() / storage_capacity)[:, :, np.newaxis])
            ...
            state_list.append((np.ones((self.n_warehouses, self.n_skus)) * (self.env.agent_states["all_warehouses", "in_transit"] * sku_profit).sum() / \
                        ((self.env.agent_states["all_warehouses", "in_transit"].sum() + 1) * price_normalize))[:, :, np.newaxis])

            # instock_intransit_sum
            state_list.append((np.ones((self.n_warehouses, self.n_skus)) * inventory_estimated.sum() / storage_capacity)[:, :, np.newaxis])

            # instock_intransit_profit
            state_list.append((np.ones((self.n_warehouses, self.n_skus)) * (inventory_estimated * sku_profit).sum() / \
